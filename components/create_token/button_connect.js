    ...
    return (
        <>
            {!active ? (
                ...
                <a onClick={disconnect} className='onLockbuttoncreate float-end mr-1' title={props.balance}><small><i className='bi bi-person-fill'></i>{truncateAddress(account)}</small></a>
            )}
        </>
    );
}
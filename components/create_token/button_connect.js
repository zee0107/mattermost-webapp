    ...
    return (
        <>
            {!active ? (
                ...
                <a onClick={disconnect} className='onLockbuttoncreate float-end mr-1'><small><i className='bi bi-person-fill' title={props.balance}></i>{truncateAddress(account)}</small></a>
            )}
        </>
    );
}